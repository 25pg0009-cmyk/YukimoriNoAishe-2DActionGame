...
int CreateSEOgg(const char* filename)
{
	...
	pse->samplesSize = decodedSize;
	pse->blockAlign = wfex.nBlockAlign;
...
void PlaySE(int seid, bool isLoop, float volume)
{
	...
	if (seid < 0 || seid >= SE_MAX) return; // 追加: 範囲チェック
 ...
 * @return 再生したブロック数
 */
uint64_t GetSECursorPos(int seid)
{
#if USE_SOUND
	SEInfo* pse = &g_se[seid];
	if (pse->count <= 0) return false;
...
/**
 * 再生した量を0～1の範囲で取得
 * ループ再生が有効な場合、ループ分だけ加算され続けるので注意
 * @param seid SEID
 * @return 開始0.0f～1.0f終了
 */
float GetSECursorPer(int seid) {
#if USE_SOUND
	SEInfo* pse = &g_se[seid];
	if (pse->count <= 0) return false;
	...
	XAUDIO2_VOICE_STATE state;
	pse->sourceVoice->GetState(&state);
	return (float)state.SamplesPlayed / (pse->samplesSize / pse->blockAlign);
#else
	return 0.f;
#endif
}
