...
//タックルエフェクトアニメーション
AnimationUniqueInfo tackleEffect_anim;
AnimationInfo tackleEffect_animptn = {
	//-1ループ,-2一度の再生で終了
	{
...
{
	bossSE[0] = CreateSEOgg("assets/SE/se_boss_fireball_00.ogg");
	bossSE[1] = CreateSEOgg("assets/SE/se_boss_tackle_00.ogg");
	isatk = false;
	demon_state = IDLE;
	...
	cooldown = 0;
	if (demon_anim.frame == 180) {
		PlaySE(bossSE[1], false);
	}
	if (demon_anim.frame == 50) {
...
void FlameDemon_Release()
{
	...
	ReleaseSE(bossSE[0]);
	ReleaseSE(bossSE[1]);
	ReleaseSpriteMatrix(tackleEffect, 24);
}
...
void FlameDemon_Update() {
 ...
 void FlameDemon_Render()
 {
	 if (demon.isAlived)
	 {
		 ...
		 RenderSpritePosLR(tackleEffect[1], 0, 0);
	 }
 }
 ...
 void Set_FireRain(float x, float y)
 {
	 for (int i = 0; i < BIGFIRE_MAX; i++)
	 {
		 if (fire_rain[i].isAlived == true)continue;
		 fire_rain[i].positionX = x;
		 fire_rain[i].positionY = y;
		 fire_rain[i].isAlived = true;
		 fire_rain[i].isAscending = true;//上昇
		 fire_rain[i].speed = R_BIGFIRE_SPEED;
	 }
 }
